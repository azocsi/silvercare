...
import {
  ...
  CheckCircle2, AlertTriangle, XCircle, Activity, Lock, ExternalLink, Zap,
} from "lucide-react";

/**
 * 사회복지사 2급 교육원 비교 v2 — "러닝화니" 스타일
 * 뉴브루탈리즘 + 베이지 크림톤. 3곳(배움·중앙·메가) F12 실측 반영.
 * v1 대비: 가격 역전 + 영상제어 청정도(감지 리스너 수) 신규 섹션.
 */

// ── 디자인 토큰 ──────────────────────────────────────────────
const B = {
  ink: "#1c1917", cream: "#fdfcf7", card: "#fffdf8", sub: "#78716c",
  line: "#1c1917", shadow: "4px 4px 0 0 #1c1917", shadowSm: "2px 2px 0 0 #1c1917",
};
const ST = {
...
const SITES = [
  {
    ...
    priceLabel: "72만", priceNum: 720000, perSubject: "정가 15만→할인",
    credit: "ok", lms: "iframe", lmsLabel: "iframe→HTML5",
    listeners: 6, ratechange: false, autoVerdict: "limited",
    login: "SecuKit NXS 공동인증서 (단 ID/PW 일반로그인 옵션 있음)",
    tested: true,
    memo: "포터 iframe(media.joongangcyber.com) → 내부는 순수 HTML5 video. 실강의 27분 풀재생됨. 배속 2.0 실측 성공. ratechange 감지 없음. 진짜 관문은 SecuKit NXS 로그인 — 단 ID/PW로 수강되면 우회 가능(확인필요). 17과목 올인원 정가 240만→70%할인→72만.",
  },
  {
    ...
    listeners: 0, ratechange: false, autoVerdict: "limited",
    login: "IP출석 1인 + 생체/OTP/PIN/SMS/PKI 대리출석 차단",
    tested: true,
    memo: "★여름초특가(6/24마감): 한방 풀패키지 86.7만(77%할인). 순수 HTML5 video. getEventListeners(video)가 완전히 비어있음(리스너 0개) — 배속·건너뛰기·탭이탈 전부 감지 안 함, 5곳 중 최청정. 단 핵심 관문은 로그인/출석(IP·생체/OTP). 영상진도가 출석으로 잡히는 구조면 그 로직 추가 확인 필요.",
  },
  {
    id: "baeoom", name: "배움사이버", url: "career.baeoom.com", tag: "✅ 가입완료",
    priceLabel: "88.24만", priceNum: 882400, perSubject: "공개 고정가",
    credit: "ok", lms: "best", lmsLabel: "HTML5 직접",
    listeners: 17, ratechange: true, autoVerdict: "limited",
    login: "SMS 본인인증 or 공동인증서 (ID/PW만으론 수강불가)",
    tested: true,
    memo: "순수 HTML5 video(iframe 없음). 단 맛보기 소스가 404라 영상 재생 자체가 안 됨 → 배속 숫자는 바뀌어도 실제 재생 못함. ratechange 등 리스너 17개 있으나 실제 막는지는 미실측(UI 동기화용일 수도). 정식 강의 재생해야 판가름. 17과목 실결제 88.24만.",
  },
  {
    id: "hackers", name: "해커스", url: "paranhanul.net", tag: "❓ 미조사",
    priceLabel: "9,700원*", priceNum: 9700, perSubject: "조건부 이벤트",
    credit: "ok", lms: "unknown", lmsLabel: "미확인",
    listeners: null, ratechange: null, autoVerdict: "unknown",
    login: "미확인", tested: false,
    memo: "다음 세션 조사. 9,700원은 7과목↑ + 최대80% 할인 조건부(정가/취소조건 확인 필수). PC·모바일 100% 연동 → HTML5 가능성↑.",
  },
  {
    id: "kstudy", name: "Kstudy", url: "kstudy.co.kr", tag: "❓ 미조사",
    priceLabel: "6.9만", priceNum: 69000, perSubject: "패키지 고정가",
    credit: "ok", lms: "unknown", lmsLabel: "미확인",
    listeners: null, ratechange: null, autoVerdict: "unknown",
    login: "미확인", tested: false,
    memo: "다음 세션 조사. 22년 평가인정. 출석률 80%+시험 60분 1회. 한 학기 최대 8과목.",
  },
  {
    id: "ubion", name: "유비온", url: "iubion.com", tag: "❓ 완전 미조사",
    priceLabel: "미조사", priceNum: 9999999, perSubject: "—",
    credit: "unknown", lms: "unknown", lmsLabel: "미확인",
    listeners: null, ratechange: null, autoVerdict: "unknown",
    login: "미확인", tested: false,
    memo: "다음 세션 처음부터 조사. 학점은행 인정 여부 먼저(❌면 탈락).",
  },
];
...
function CleanlinessBar({ listeners }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <span style={{ fontSize: 12.5, fontWeight: 900, color: B.ink, minWidth: 54, textAlign: "right" }}>
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 880, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 22 }}>
          ...
          }}>🎓 사회복지사 2급 · S1 ch2 · LMS 자동화 실측</span>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 13, fontWeight: 800, color: "#fbbf24", marginBottom: 6 }}>📌 실측 결론 (v1에서 2개 뒤집힘)</div>
          <div style={{ fontSize: 15, fontWeight: 600, lineHeight: 1.6 }}>
            <b style={{ color: "#6ee7b7" }}>가격</b>: 중앙 72만이 실결제 최저 (v1의 "가장 비쌈" 뒤집힘). 메가 86.7만 프로모션.
            <b style={{ color: "#6ee7b7" }}> 영상제어</b>: <b>메가가 감지 리스너 0개로 가장 깨끗</b> (v1의 "가장 빡셈" 뒤집힘).
            <b style={{ color: "#fbbf24" }}> 단 3곳 다 ⚠️ 부분</b> — 진짜 관문은 로그인/출석이고, 진도저장은 정식 강의실에서만 확인 가능.
            <b style={{ color: "#fda4af" }}> 17과목 풀 → 3학기 → 발급 ~2028 상반기는 불변.</b>
          </div>
        </div>

        {/* ── 01 핵심 3축 ── */}
        <div style={{ marginBottom: 30 }}>
          <SectionTitle no="01" title="실측 3줄 요약" sub="가설 말고 직접 찍은 것" />
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 12 }}>
            {[
              { ic: <DollarSign size={20} />, t: "최저가", v: "중앙 72만", d: "정가 240만→70%할인. 메가 86.7만 프로모션이 2위", s: "best" },
              { ic: <Zap size={20} />, t: "영상제어 최청정", v: "메가 리스너 0개", d: "배속·건너뛰기·탭이탈 전부 감지 안 함", s: "best" },
        ...
        {/* ── 02 영상제어 청정도 (신규 핵심 섹션) ── */}
        <div style={{ marginBottom: 30 }}>
          <SectionTitle no="02" title="영상제어 청정도" sub="감지 리스너가 적을수록 자동화 유리 (실측한 3곳만)" />
          <Card>
            {tested.map((s, i) => (
            ...
            <div style={{ marginTop: 12, fontSize: 12, color: B.sub, fontWeight: 600, lineHeight: 1.5 }}>
              ※ <b>메가 0개</b> = getEventListeners(video)가 완전히 비어있음(배속/건너뛰기 감지 전무). <b>중앙 6개</b> = ratechange 없어 배속 막지 않음. <b>배움 17개</b> = ratechange 있으나 맛보기 404로 실제 작동 미확인.
            </div>
          </Card>
        </div>
        ...
        {/* ── 03 가격 랭킹 ── */}
        <div style={{ marginBottom: 30 }}>
          <SectionTitle no="03" title="실결제 가격 랭킹" sub="17과목 기준 (해커스 조건부·유비온 미조사 제외)" />
          <Card>
            {priceRank.map((s, i) => {
              const max = 882400;
            ...
            <div style={{ marginTop: 10, fontSize: 12, color: B.sub, fontWeight: 600 }}>
              ※ 메가 86.7만은 여름초특가(6/24 마감). 중앙·메가 모두 상담 쿠폰 변수 있어 최종 결제 직전 재확인 필수. 총비용엔 실습세미나·실습기관비·행정비 별도.
            </div>
          </Card>
        </div>
        ...
        {/* ── 04 비교 매트릭스 ── */}
        <div style={{ marginBottom: 30 }}>
          <SectionTitle no="04" title="교육원별 상세" sub="행을 누르면 펼쳐져요 · 실측 3곳 + 미조사 3곳" />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {SITES.map((s) => {
              ...
              return (
                <div key={s.id} style={{ background: B.card, border: `2px solid ${B.ink}`, borderRadius: 14, boxShadow: isOpen ? B.shadow : B.shadowSm, opacity: s.tested ? 1 : 0.82 }}>
                  <button
                  ...
                  >
                    ...
                    <span style={{ marginLeft: "auto", display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap", justifyContent: "flex-end" }}>
                      ...
                      <Chip status={s.autoVerdict} text={s.tested ? (s.autoVerdict === "limited" ? "⚠️ 부분" : ST[s.autoVerdict].label) : "자동화 ?"} />
                    </span>
                  </button>
                  ...
                  {isOpen && (
                    <div style={{ padding: "0 16px 16px", borderTop: `1.5px dashed #d6ccb5` }}>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 8, margin: "14px 0" }}>
                        {[
                          ["학점은행", <Chip status={s.credit} />],
                          ["LMS", <Chip status={s.lms} text={s.lmsLabel} />],
                          ["감지 리스너", <span style={{ fontSize: 14, fontWeight: 900, color: B.ink }}>{s.listeners === null ? "미측정" : `${s.listeners}개`}</span>],
                          ["과목당", <span style={{ fontSize: 13, fontWeight: 800, color: B.ink }}>{s.perSubject}</span>],
                        ].map(([label, node], i) => (
                          <div key={i} style={{ background: B.cream, border: `1.5px solid ${B.ink}`, borderRadius: 10, padding: "8px 10px" }}>
                      ...
                      <div style={{ display: "flex", alignItems: "flex-start", gap: 7, marginBottom: 10, background: "#fef3c7", border: `1.5px solid ${B.ink}`, borderRadius: 10, padding: "9px 11px" }}>
                        <Lock size={15} style={{ marginTop: 2, flexShrink: 0 }} />
                        <div>
                          <div style={{ fontSize: 11, fontWeight: 800, color: "#b45309" }}>로그인 장벽 (진짜 관문)</div>
        ...
        {/* ── 05 냉정한 한계점 ── */}
        <div style={{ marginBottom: 30 }}>
          <SectionTitle no="05" title="냉정한 한계점" sub="실측했어도 아직 모르는 것" />
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(250px,1fr))", gap: 12 }}>
            {[
              { ic: <Activity size={18} color="#b45309" />, t: "진도저장은 3곳 다 미확인", d: "맛보기는 부모페이지 연결이 끊겨 Network 측정 불가. 정식 로그인 강의실에서 Fetch/XHR 확인해야 최종 ✅/❌." },
              { ic: <Lock size={18} color="#be123c" />, t: "영상 깨끗 ≠ 자동화 가능", d: "메가는 영상 리스너 0개지만 로그인=IP+생체/OTP. 영상제어보다 로그인/출석이 진짜 관문." },
              { ic: <AlertTriangle size={18} color="#b45309" />, t: "배움은 실측 자체가 안 됨", d: "맛보기 소스 404로 영상이 안 돌아감. ratechange 17개도 실제 막는지 미확인. 정식강의 재검증 필수." },
              { ic: <XCircle size={18} color="#1d4ed8" />, t: "아직 3곳 남음", d: "해커스·Kstudy·유비온 미조사. 해커스 9,700원 함정 + 유비온 학점은행 인정부터 확인해야." },
            ].map((x, i) => (
        ...
        <div style={{ marginBottom: 24 }}>
          <SectionTitle no="06" title="다음 세션 할 일" sub="순서대로" />
          <Card>
            {[
              ["해커스 F12 + 9,700원 함정 확인", "7과목↑·최대80% 조건. 정가/취소조건 / paranhanul.net"],
              ["Kstudy F12", "출석률 80%+시험 60분. 영상 LMS만 / kstudy.co.kr"],
              ["유비온 처음부터 조사", "학점은행 인정 먼저(❌면 탈락) / iubion.com"],
              ["최종 비교표 → GitHub silvercare ch2 푸시", "6곳 다 모인 뒤 가격·자동화 최종 갱신"],
              ["기관 확정 → 2026 2학기 8과목 신청", "최저가×영상청정×로그인난이도 교차점 (시나리오 B)"],
            ].map(([t, d], i) => (
        ...
        <div style={{ textAlign: "center", fontSize: 11.5, color: "#a8a29e", fontWeight: 700, marginTop: 28 }}>
          🥈 사회복지사 2급 · Season 1 Chapter 2 · LMS 자동화 실측 · 배움·중앙·메가 완료
        </div>
      </div>
    </div>
  );
